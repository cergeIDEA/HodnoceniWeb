...
data = pd.read_excel('data/Institutions_Final.xlsx',sheet_name='JEDNOTKA')
...
for predkl in data.Predkladatel_long.unique():
    ...
    d2['Type'] = str(data[data.Predkladatel_long == predkl].Type.iloc[0])
    d2['level'] = 0
    l.append(d2)
    ...
    for index,row in data[data.Predkladatel_long == predkl].iterrows():
        ...
        d3['Type'] = str(row.Type)
        d3['Field'] = str(row.Obor)
...
with open('data/ddldata.js', 'w', encoding='utf-8') as f:
...
with open('data/institutions.js','w',encoding='utf-8') as f:
    f.write(str(s0 + s1+';\n' + s2 + s3 ))
